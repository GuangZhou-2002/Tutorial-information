...
(function (factory) {
  ...
  $.fn.citys = function (parameter, getApi) {
    ...
    var defaults = {
      dataUrl: 'https://passer-by.com/data_location/list.json',
      //数据库地址
      crossDomain: true,
      //是否开启跨域
      dataType: 'json',
          ...
          var format = {
            province: function province() {
              ...
              for (var i in _province) {
                $province.append('<option value="' + (options.valueType == 'code' ? i : _province[i]) + '" data-code="' + i + '">' + _province[i] + '</option>');
              }

              if (options.province) {
                var value = options.valueType == 'code' ? options.province : _province[options.province];
                $province.val(value);
              }
            ...
            city: function city() {
              ...
              if (!hasCity) {
                ...
                for (var i in _city) {
                  $city.append('<option value="' + (options.valueType == 'code' ? i : _city[i]) + '" data-code="' + i + '">' + _city[i] + '</option>');
                }

                if (options.city) {
                  var value = options.valueType == 'code' ? options.city : _city[options.city];
                  $city.val(value);
                } else if (options.area) {
                  var value = options.valueType == 'code' ? options.area : _city[options.area];
                  $city.val(value);
                }
              }
            ...
            area: function area() {
              ...
              for (var i in _area) {
                $area.append('<option value="' + (options.valueType == 'code' ? i : _area[i]) + '" data-code="' + i + '">' + _area[i] + '</option>');
              }

              if (options.area) {
                var value = options.valueType == 'code' ? options.area : _area[options.area];
                $area.val(value);
              }
            }
          }; //获取当前地理信息
